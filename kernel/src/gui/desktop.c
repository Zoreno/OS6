...
unsigned int mouse_img[MOUSE_BUFSZ] = {
    cX, c_, c_, c_, c_, c_, c_, c_, c_, c_, c_, cX, cX, c_, c_, c_, c_, c_,
    c_, c_, c_, c_, cX, cO, cX, c_, c_, c_, c_, c_, c_, c_, c_, cX, cO, cO,
    cX, c_, c_, c_, c_, c_, c_, c_, cX, cO, cO, cO, cX, c_, c_, c_, c_, c_,
    c_, cX, cO, cO, cO, cO, cX, c_, c_, c_, c_, c_, cX, cO, cO, cO, cO, cO,
    cX, c_, c_, c_, c_, cX, cO, cO, cO, cO, cO, cO, cX, c_, c_, c_, cX, cO,
    cO, cO, cO, cO, cO, cO, cX, c_, c_, cX, cO, cO, cO, cO, cO, cO, cO, cO,
    cX, c_, cX, cO, cO, cO, cO, cO, cO, cO, cO, cO, cX, cX, cO, cO, cX, cO,
    cO, cO, cX, cX, cX, cX, cX, cO, cX, c_, cX, cO, cO, cX, c_, c_, c_, cX,
    cX, c_, c_, cX, cO, cO, cX, c_, c_, c_, cX, c_, c_, c_, c_, cX, cO, cO,
    cX, c_, c_, c_, c_, c_, c_, c_, cX, cO, cO, cX, c_, c_, c_, c_, c_, c_,
    c_, c_, cX, cO, cX, c_, c_, c_, c_, c_, c_, c_, c_, c_, cX, cX, c_, c_};

#define TASKBAR_HEIGHT 32
#define TASKBAR_ICON_WIDTH 128

void Desktop_paint_taskbar(Window *taskbar_window);
void Desktop_taskbar_process_mouse(Window *taskbar_window,
                                   int mouse_x,
                                   int mouse_y,
    ...
    if (taskbar_window->last_button_state != mouse_buttons)
    {
        for (i = 0; i < desktop_window->children->count; ++i)
        {
            ...
            if (mouse_x > (48 + (TASKBAR_ICON_WIDTH + 10) * current_index) &&
                mouse_x <= (48 + (TASKBAR_ICON_WIDTH + 10) * current_index +
                            TASKBAR_ICON_WIDTH) &&
                mouse_y > 6 && mouse_y <= 26)
            {
...
void Desktop_paint_taskbar(Window *taskbar_window)
{
    ...
    for (i = 0; i < desktop_window->children->count; ++i)
    {
        ...
        active_child = desktop_window->active_child &&
                       (desktop_window->active_child == child);
...
void Desktop_taskbar_tick_handler(Window *taskbar_window, int ticks)
{
    ...
    Window_invalidate(taskbar_window,
                      0,
                           ...
                           uint8_t mouse_buttons)
{
    ...
    if (!(mouse_rect = rect_new(desktop->mouse_y,
                                desktop->mouse_x,
        ...
        for (x = 0; x < MOUSE_WIDTH; ++x)
        {
            ...
            if (mouse_img[y * MOUSE_WIDTH + x] & 0xFF000000)
            {
                desktop->window.context
                    ->buffer[(y + mouse_y) * desktop->window.context->width +
                             (x + mouse_x)] = mouse_img[y * MOUSE_WIDTH + x];
            }
        }
    }
}
...
{
    Window *new_window =
        Window_create_window(desktop_window, x, y, width, height, flags);
