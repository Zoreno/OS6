...
static const uint32_t rcon[11] = {0x00000000,
                                  0x00000001,
                                  0x00000002,
                                  0x00000004,
                                  0x00000008,
...
static const crypto_cipher_algo_t aes_algo = {
    ...
    CIPHER_TYPE_BLOCK,
    AES_BLOCK_SIZE,
    (cipher_algo_init_func_t)aes_init,
    NULL,
    NULL,
...
static size_t get_rounds(size_t key_len);
static void gen_enc_key_schedule(aes_context_t *context,
                                 const uint8_t *key,
                                 size_t key_len);
static void gen_dec_key_schedule(aes_context_t *context,
                                 const uint8_t *key,
                                 size_t key_len);
...
static void gen_enc_key_schedule(aes_context_t *context,
                                 ...
                                 size_t key_len)
{
    size_t key_schedule_size = 4 * (context->nr + 1);
    ...
    for (size_t i = key_len; i < key_schedule_size; ++i)
    {
        uint32_t prev_word = context->ek[i - 1];

        if ((i % key_len) == 0)
        {
            context->ek[i] = sbox[(prev_word >> 8) & 0xFF];
            context->ek[i] |= (sbox[(prev_word >> 16) & 0xFF] << 8);
            context->ek[i] |= (sbox[(prev_word >> 24) & 0xFF] << 16);
            context->ek[i] |= (sbox[prev_word & 0xFF] << 24);
            context->ek[i] ^= rcon[i / key_len];
        }
        else if (key_len > 6 && (i % key_len) == 4)
        {
            context->ek[i] = sbox[prev_word & 0xFF];
            context->ek[i] |= (sbox[(prev_word >> 8) & 0xFF] << 8);
            context->ek[i] |= (sbox[(prev_word >> 16) & 0xFF] << 16);
            context->ek[i] |= (sbox[(prev_word >> 24) & 0xFF] << 24);
        }
        else
        {
            context->ek[i] = prev_word;
        }

        context->ek[i] ^= context->ek[i - key_len];
    }
}
                                 ...
                                 size_t key_len)
{
    ...
    for (size_t i = 0; i < key_schedule_size; ++i)
    {
        ...
        else
        {
            ...
            context->dk[i] ^= rotate_left32(temp, 8);
            temp = td[sbox[(context->ek[i] >> 16) & 0xFF]];
            context->dk[i] ^= rotate_left32(temp, 16);
            temp = td[sbox[(context->ek[i] >> 24) & 0xFF]];
            context->dk[i] ^= rotate_left32(temp, 24);
        }
    }
}
...
error_t aes_init(aes_context_t *context, const uint8_t *key, size_t key_len)
{
    ...
    // The next functions assumes key_len is in 32-bit words.
    key_len /= 4;

    gen_enc_key_schedule(context, key, key_len);
    gen_dec_key_schedule(context, key, key_len);
                       ...
                       uint8_t *output)
{
    ...
    for (i = 1; i < context->nr; i++)
    {
        ...
        t0 = te[s0 & 0xFF];
        temp = te[(s1 >> 8) & 0xFF];
        t0 ^= rotate_left32(temp, 8);
        temp = te[(s2 >> 16) & 0xFF];
        t0 ^= rotate_left32(temp, 16);
        temp = te[(s3 >> 24) & 0xFF];
        t0 ^= rotate_left32(temp, 24);

        t1 = te[s1 & 0xFF];
        temp = te[(s2 >> 8) & 0xFF];
        t1 ^= rotate_left32(temp, 8);
        temp = te[(s3 >> 16) & 0xFF];
        t1 ^= rotate_left32(temp, 16);
        temp = te[(s0 >> 24) & 0xFF];
        t1 ^= rotate_left32(temp, 24);

        t2 = te[s2 & 0xFF];
        temp = te[(s3 >> 8) & 0xFF];
        t2 ^= rotate_left32(temp, 8);
        temp = te[(s0 >> 16) & 0xFF];
        t2 ^= rotate_left32(temp, 16);
        temp = te[(s1 >> 24) & 0xFF];
        t2 ^= rotate_left32(temp, 24);

        t3 = te[s3 & 0xFF];
        temp = te[(s0 >> 8) & 0xFF];
        t3 ^= rotate_left32(temp, 8);
        temp = te[(s1 >> 16) & 0xFF];
        t3 ^= rotate_left32(temp, 16);
        temp = te[(s2 >> 24) & 0xFF];
        t3 ^= rotate_left32(temp, 24);
