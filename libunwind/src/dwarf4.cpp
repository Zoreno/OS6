...
public:
    ...
    cfi_register(uint64_t index, register_rules rule, uint64_t value) : m_index(index),
                                                                        m_rule(rule),
                                                                        m_value(value)
    {
    }
...
static uint64_t private_parse_expression(char *p,
...
static uint64_t private_decode_cfa(const cfi_table_row &row, register_state *state)
{
    uint64_t value = 0;
...
static uint64_t private_decode_reg(const cfi_register &reg, uint64_t cfa, register_state *state)
{
    uint64_t value = 0;
...
static void private_parse_instructions(cfi_table_row *row,
                                       const ci_entry &cie,
                                       const fd_entry &fde,
                                       register_state *state,
                                       bool is_cie)
...
void dwarf4::unwind(const fd_entry &fde, register_state *state)
{
    if (state == nullptr)
    {
        return;
